...
export const COST = {
   ...
   * = "이만큼 조용하면 말이 끝난 것으로 보고 서버에 보낸다"
   *
   * ⚠️ 2026-08-19 사고 — "일정 길이 문장까지만 듣는다 /
   *    내가 하지 않은 말(dying)이 중간에 나온다"
   *
   *    2400ms 였습니다. 그런데 **한국 사람이 영어로 말할 때**는
   *    문장 중간에 그보다 오래 뜸을 들입니다. 단어가 안 떠오르거나
   *    문장 구조를 고민할 때입니다.
   *
   *      "The weather was ........ dying"
   *                        ↑ 2.4초 넘음 → 여기서 턴이 끊김
   *
   *    그러면 앞조각("The weather was")이 한 턴으로 올라가고,
   *    뒷조각("dying")이 **별도 발화**로 올라갑니다. 화면에는
   *    학생이 "dying" 한 단어만 말한 것처럼 찍히고, 앞조각에는
   *    선생님이 엉뚱하게 "That was really good!" 이라고 답합니다.
   *    목사님이 "하지 않은 말이 나온다"고 하신 것이 이것입니다.
   *
   *    3200ms 로 늘립니다. 대답이 0.8초 늦어지는 대신 문장이
   *    통째로 갑니다. **말이 잘리는 것이 느린 것보다 훨씬 나쁩니다** —
   *    잘리면 학습자는 자기가 뭘 잘못했는지도 모릅니다.
   *
   *    (쉬었다가 다시 말하면 타이머가 초기화되므로, 이 값을 늘려도
   *     "말을 마쳤는데 한참 기다리는" 일은 늘지 않습니다.
   *     늘어나는 건 문장 중간에 뜸 들일 때 참아주는 여유뿐입니다.)
   */
  SILENCE_TAIL_MS: 3200,
