 ...
 */
export const APP_VERSION = 'v11-voice-floor';
...
export const COST = {
  ...
  /**
   * 발화 문턱의 **절대 천장.** 무슨 일이 있어도 문턱은 이 위로 못 올라갑니다.
   *
   * ⚠️ 2026-08-18. 예전에는 0.055 였습니다. 그런데 보통 대화 목소리의 RMS 는
   *    0.02~0.05 입니다. 즉 천장이 **사람 목소리보다 위에 있었습니다.**
   *    소음 학습이 한 번 잘못되면 문턱이 목소리 위로 올라가버리고,
   *    그러면 마이크가 통째로 먹통이 됩니다. 실제로 그 일이 났습니다.
   *    (계측: 6초 연속 발화 뒤 문턱 0.0468 → 보통 목소리 0.025 로 안 열림)
   *
   *    이제 천장을 **사람 목소리 아래**로 내립니다. 이렇게 하면 소음 추정에
   *    어떤 사고가 나도 **보통 목소리는 언제나 게이트를 엽니다.**
   *    피해의 상한을 코드로 못 박아두는 것입니다.
   *
   *    부작용: 밑소음이 이 값보다 큰 아주 시끄러운 방에서는 게이트가 계속
   *    열려 있게 됩니다. 그건 speechEnergy.js 가 막습니다 — 말소리가 350ms
   *    미만이면 서버로 보내지 않고 화면에 이유를 표시합니다. 방어가 두 겹입니다.
   */
  MAX_SPEECH_RMS: 0.018,
  ...
  /**
   * 소음 추정이 이 시간 동안 한 번도 갱신되지 않으면 "굶었다"고 보고 구제합니다.
   *
   * ⚠️ 2026-08-18. 이 장치를 한 번 통째로 지웠다가 되돌렸습니다.
   *    "사람 목소리를 소음으로 배운다"고 의심했지만, 그 판단의 근거였던
   *    합성음이 6초 내내 세기가 평평한 비현실적인 소리였습니다.
   *    실제 말은 음절 사이에서 세기가 크게 꺼지므로 이 구조는 안전합니다.
   *    지웠더니 회귀 테스트 [7]이 3건 무너졌습니다 — 굶주림이 되살아났습니다.
   *    진짜 범인은 문턱에 **천장이 없었던 것**이었습니다. @see MAX_SPEECH_RMS
   */
